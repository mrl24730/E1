...
//        }
        ...
        //Godfrey 
        //Log.d("Total Page", "Total Page : " + totalPage);
        ...
        if (mFirstLayout) {
            ...
            mNextScreen = Math.max(0, Math.min(getCurrentScreen(), totalPage - 1));// Godfrey getChildCount() - 1));
                    ...
                    if (deltaX < 0) {
                        ...
                        final int availableToScroll =
                              //  getChildAt(getChildCount() - 1).getRight() - scrollX - getWidth();
                        		//Godfrey
                        		  getChildAt(totalPage - 1).getRight() - scrollX - getWidth();
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
        	...
        	//Godfrey 
        	mCurrentScreen = Math.max(0, Math.min(mNextScreen, totalPage - 1));
    ...
    public void setCurrentScreen(final int currentScreen, final boolean animate) {
    	...
    	//Godfrey 
    	mCurrentScreen = Math.max(0, Math.min(currentScreen, totalPage - 1));
    ...
    private void snapToDestination() {
        ...
        } else if ((deltaX > 0) && (mCurrentScreen + 1 != totalPage)  //Godfrey getChildCount()
                && ((screenWidth / FRACTION_OF_SCREEN_WIDTH_FOR_SWIPE) < deltaX)) {
            whichScreen++;
        }
    ...
    private void snapToScreen(final int whichScreen, final int duration) {
         ...
         */
    	//Godfrey 
    	mNextScreen = Math.max(0, Math.min(whichScreen, totalPage - 1));
    ...
    //ivan
    public void setCanClick(Boolean _canClick)
    {
    	canClick = _canClick;
    }
}
